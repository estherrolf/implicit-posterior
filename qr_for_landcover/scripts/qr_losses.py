...
def loss_on_prior_simple(probs: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    """Computes the QR (forwards) loss on prior.

    Args:
        probs: pre-normalized probabilities of predictions, expected shape B x C x H x W.
...
def loss_on_prior_reversed_kl_simple(
    ...
    Args:
        lprobs: pre-normalized probabilities of predictions, expected shape B x C x H x W.
        target: prior probabilities, expected shape B x C x H x W
