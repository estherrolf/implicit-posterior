...
run_dirs = [
   ...
   # 'ea_from_pittsburgh_model',
   # 'ea_from_scratch',
   # 'ea_learned_prior'
]

loss_to_eval_by_run = {
    'hp_gridsearch_pittsburgh': ['ce'],
    'hp_gridsearch_pittsburgh_with_prior_as_input': ['ce'],
    'ea_from_pittsburgh_model': ['qr_forward', 'qr_reverse'],
    'ea_from_scratch': ['qr_forward', 'qr_reverse'],
    'ea_learned_prior': ['qr_forward']
}
    ...
    for states_str in states_to_eval:
        for loss in loss_to_eval_by_run[run_dir]:
            ...
            # hr model take the best from the grid search runs
                ...
                run_name = 'pittsburgh_pa-2010_1m_fcn_0.0001_ce/'
            ...
            data_dir_this_state = f'{torchgeo_data_dir}/enviroatlas/{states_str}-test_tiles-debuffered'
            ...
            if not os.path.exists(f'{torchgeo_pred_dir}/{run_name}/enviroatlas'):
                os.mkdir(f'{torchgeo_pred_dir}/{run_name}/enviroatlas')
                print(f'making dir {torchgeo_pred_dir}/{run_name}/enviroatlas')
            if not os.path.exists(f'{torchgeo_pred_dir}/{run_name}/enviroatlas/{states_str}-test_tiles-debuffered'):
                os.mkdir(f'{torchgeo_pred_dir}/{run_name}/enviroatlas/{states_str}-test_tiles-debuffered')
                print(f'making dir {torchgeo_pred_dir}/{run_name}/enviroatlas/{states_str}-test_tiles-debuffered')
